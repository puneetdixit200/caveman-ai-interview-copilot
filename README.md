...
- Store distribution and installer polish beyond signed GitHub Releases
...
Windows Authenticode signing is required for redistributable commercial Windows installers. Add `WINDOWS_CODESIGN_CERTIFICATE_BASE64` and `WINDOWS_CODESIGN_CERTIFICATE_PASSWORD` repository secrets to import a `.pfx` certificate during the release workflow, or set `WINDOWS_CODESIGN_CERTIFICATE_THUMBPRINT`/`WINDOWS_CODESIGN_SIGN_COMMAND` before running `npm run tauri:build:signed` locally on Windows. The build script injects Tauri's Windows signing config before updater artifacts are created so installer signatures and updater signatures are generated in the same release pass.

macOS release jobs expect Apple Developer signing and notarization secrets before they publish signed updater archives and DMG installers. Add `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, and `KEYCHAIN_PASSWORD` for certificate import. For notarization, either add Apple ID credentials with `APPLE_ID`, `APPLE_PASSWORD`, and `APPLE_TEAM_ID`, or use App Store Connect API credentials with `APPLE_API_ISSUER`, `APPLE_API_KEY`, and `APPLE_API_PRIVATE_KEY_BASE64`. The workflow signs each generated DMG, submits it with `notarytool`, staples the notarization ticket, validates the staple, and runs Gatekeeper assessment before upload. The macOS bundle includes audio privacy usage descriptions for microphone and system-audio capture.
...
After packaging, run `npm run package:verify-sidecar` and `npm run package:verify-privacy-shield` on the same platform to inspect the redistributable output. The verifiers check the macOS app bundle, extract the Windows MSI payload, inspect Linux DEB/AppImage packages for the bundled `caveman-whisper` runtime, and require packaged native plus built-frontend privacy shield markers before artifacts are uploaded in CI or published by the signed release workflow. On macOS, `npm run dmg-meeting-risk:smoke:mac` mounts the generated DMG and runs the Google Meet/Teams meeting-risk simulation against the app inside the mounted installer.
