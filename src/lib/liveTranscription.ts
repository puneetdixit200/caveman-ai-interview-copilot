...
export async function runLiveTranscriptionPass(input: {
  ...
  const snapshot = await deps.saveCaptureSnapshot({
    source: selectSnapshotSource(input.config.audio.captureMode),
    maxSeconds: input.maxSeconds ?? 6
  });
  ...
  for (const event of events) {
    ...
    const key = transcriptKey(event.speaker, text);
...
function selectSnapshotSource(captureMode: AudioCaptureMode): SnapshotSource {
  return captureMode === "system" || captureMode === "dual" ? "system" : "microphone";
}
